...
* @param {function(!Element=):string=} titleFunc A function that extracts the caption text corresponding to an element to be shown.
*/
	...
	for (let i = 0, m = index; i < rows; ++i, m += majorstride) {
		for (let j = 0, n = m; j < cols; ++j, n += minorstride) {
			...
			if (assignable) {
				...
				captionholder.innerHTML = decodeEntities(caption || '');
				if (caption) {
